...
int main()
{
	...
	while (true)
	{
		...
		// Find total CPU time, then create percentages
		long long totalUsageTime = cpuUsageReport.IdleTime().count() + cpuUsageReport.KernelTime().count() + cpuUsageReport.UserTime().count();
		double percentIdleTime = (static_cast<double>(cpuUsageReport.IdleTime().count()) / totalUsageTime) * 100;
		double percentKernelTime = (static_cast<double>(cpuUsageReport.KernelTime().count()) / totalUsageTime) * 100;
		double percentUserTime = (static_cast<double>(cpuUsageReport.UserTime().count()) / totalUsageTime) * 100;

		std::wcout << "Percent of CPU time idling:          " << percentIdleTime << "% \n";
